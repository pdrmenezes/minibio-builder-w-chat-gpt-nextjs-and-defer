...
export async function getGithubInfo(githubUsername: string): Promise<GitHubInfo> {
...
export async function generateGitHubProfile(githubUsername: string) {
  ...
  - Wrote ${Object.entries(languages)
    .map(([name, count]) => (name === "total" || (count / languages.total) * 100 < 1 ? "" : `${Math.ceil((count / languages.total) * 100)}% ${name}`))
    .filter((v) => !!v)
    .join(", ")}
